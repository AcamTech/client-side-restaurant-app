...
export default createClass({
  ...
  render(){
    var {openCreateRestaurantModal, closeCreateRestaurantModal, isOpen} = this.props;
    return(
      <button onClick={openCreateRestaurantModal} className="button button--primary">
        {'Registrar Restaurante'}
        <Modal
          isOpen={isOpen}
          onCloseHandler={closeCreateRestaurantModal}>
          <article id="new-restaurant" className="panel panel--full-space panel--medium mfp-with-anim" style={{clear: 'both'}}>
            ...
            <RestaurantForm onAfterSubmitHandler={closeCreateRestaurantModal} onCancelClick={closeCreateRestaurantModal} />
          </article>
        </Modal>
      </button>
    );
